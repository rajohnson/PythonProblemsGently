...
def test_chess_black(row: int, col: int) -> None:
    assert getChessSquareColor(row, col) == "black"


@pytest.mark.parametrize("row, col", [(0, 0), (7, 1), (7, 7)])
def test_chess_white(row: int, col: int) -> None:
    assert getChessSquareColor(row, col) == "white"


@pytest.mark.parametrize("row, col", [(0, 9), (0, 8), (0, -1), (-1, 5), (8, 5)])
def test_chess_bad_square(row: int, col: int) -> None:
